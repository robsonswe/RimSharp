...
namespace RimSharp.Features.WorkshopDownloader.Services
{
    ...
    public class WebNavigationService : IWebNavigationService
    {
        ...
        {
            // Update button states etc. *after* navigation attempt completes
            UpdateNavigationState();
            ...
            if (e.IsSuccess)
            {
                ...
                CheckUrlValidity(currentSource); // Check URL validity on successful load
            }
            else
            {
                status = $"Failed to load: {currentSource} (Error: {e.WebErrorStatus})";
                 // Consider if a failed URL is ever a "valid" mod URL (probably not)
                 CheckUrlValidity(null); // Treat failed load as invalid URL
            }
