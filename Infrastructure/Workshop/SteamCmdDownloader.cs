...
namespace RimSharp.Infrastructure.Workshop
{
    public class SteamCmdDownloader : ISteamCmdDownloader
    {
        ...
        // private static readonly Regex GenericErrorRegex = new Regex( // Unused currently
        //     @"^ERROR! (.*)",
        //     RegexOptions.Compiled | RegexOptions.IgnoreCase);
                ...
                CancellationToken cancellationToken = default)
        {
            ...
            {
                _logger.LogError("Download failed: SteamCMD is not set up properly (missing Exe path or failed setup check)", "SteamCmdDownloader");
            ...
            if (!validItems.Any())
            {
                ...
                result.OverallSuccess = true; // No work needed, so technically successful.
                return result;
            }

            foreach (var item in validItems)
            {
                // Assuming SteamId is non-null here due to the filter above
                _logger.LogDebug($"Preparing to download item: {item.SteamId!} - {item.Name}", "SteamCmdDownloader");
            }

            // Explicitly type the dictionary to ensure non-nullable key.
            var itemLookup = validItems.ToDictionary<DownloadItem, string>(i => i.SteamId!);

            var requestedIds = itemLookup.Keys.ToHashSet<string>();
            ...
            string? installPath = _pathService.SteamCmdInstallPath;
            if (string.IsNullOrEmpty(installPath))
            {
                _logger.LogError("Error: SteamCMD install path is not configured.", "SteamCmdDownloader");
                result.LogMessages.Add("Error: SteamCMD install path is not configured.");
                result.OverallSuccess = false;
                // Note: Script file hasn't been created yet, so no cleanup needed here.
                return result;
            }
            string workshopLogPath = Path.Combine(installPath, "logs", "workshop_log.txt");
            ...
            try
            {
                ...
                scriptBuilder.AppendLine($"force_install_dir \"{_pathService.SteamCmdSteamAppsPath}\""); // Assuming SteamAppsPath is valid if install path is
                ...
                foreach (var item in validItems)
                {
                    // SteamId is guaranteed non-null here by the initial filter
                ...
                {
                    FileName = _pathService.SteamCmdExePath, // Already checked for null/empty
                    Arguments = $"+runscript \"{scriptPath}\" +log_file \"{primaryLogPath}\"",
                    UseShellExecute = true, // Keep false if you want to redirect output/error later
                    RedirectStandardOutput = false,
                    RedirectStandardError = false,
                    CreateNoWindow = false, // Set true for silent execution, but might hide issues
                    ...
                    try
                    {
                        ...
                        foreach (var line in logLines)
                        {
                            ...
                            if (successMatch.Success)
                            {
                                ...
                                string id = successMatch.Groups[2].Value; // ID is non-nullable string here
                                ...
                                {
                                    // requestedIds contains non-nullable strings
                            ...
                            if (!matched)
                            {
                                ...
                                if (failureMatch.Success)
                                {
                                    ...
                                    string id = failureMatch.Groups[2].Value; // ID is non-nullable string here
                ...
                {
                    // Workshop log unusable, rely solely on exit code (or lack thereof)
                    ...
                    {
                        // Exit code 0 is ambiguous without the log
                ...
                // itemLookup has non-nullable keys (string)
                ...
                // Requires exit code 0 AND all requested items succeeded based on *this session's* log entries.
                ...
                {
                     // Already logged / added message above when determining failure reason
                     ...
                     // result.LogMessages already contains info about log issue
                }
                 else if (failedIds.Any()) // Check failedIds directly
                {
                ...
                {
                    // This case should now be covered by the `failedIds.Any()` check above,
                    // because items without log entries are added to failedIds.
                    // Retaining the log message for robustness, though it might be redundant.
            ...
            catch (OperationCanceledException)
            {
                ...
                var determinedIds = sessionResults.Keys.ToHashSet<string>(); // sessionResults keys are non-null string
                result.FailedItems.AddRange(validItems.Where(i => !determinedIds.Contains(i.SteamId!)));
                // Ensure items that *did* succeed before cancellation are still marked as succeeded
                result.SucceededItems.AddRange(sessionResults.Where(kvp => kvp.Value.Success).Select(kvp => itemLookup[kvp.Key]));
                // Remove succeeded items from the failed list if they were added by the broad filter above
                result.FailedItems.RemoveAll(item => result.SucceededItems.Contains(item));
            ...
            catch (Exception ex)
            {
                ...
                result.OverallSuccess = false;
                // Ensure all requested items not already marked succeeded are marked failed
                var succeededIds = result.SucceededItems.Select(i => i.SteamId).ToHashSet();
                result.FailedItems.AddRange(validItems.Where(i => i.SteamId != null && !succeededIds.Contains(i.SteamId))); // Check i.SteamId != null for safety
